...
function updateGame() {
    ...
    // G√©n√©rer des void particles en orbite
    if (Math.random() < 0.3) {
    ...
    // Met √† jour et dessine les folders (update d'abord, draw ensuite pour le z-index)
    for (const folder of folders) {
    ...
    // Supprimer les folders aspir√©s visuellement
        ...
        for (let j = mobs.length - 1; j >= 0; j--) {
            ...
            if (bullet.hits(mob)) {
                ...
                if (mob.hp <= 0) {
                    ...
                    const scrapCount = mob.nutrition; // ou fixe ‡ 1 si pas de nutrition
                    ...
                    for (let s = 0; s < scrapCount; s++) {
                        flyingScraps.push({
                            ...
                            vy: -Math.random() * 2 - 1,
                            alpha: 1,
                            delay: s * 5, // Ètalement de l'apparition des scraps
                            reached: false
                        });
                    }
                }
    ...
    // Animation des scraps collectÈs
    const targetX = canvas.width - 90;
    const targetY = 32;

    const scrapImgCollect = new Image();
    scrapImgCollect.src = "assets/scrapCollect.png"; // Scrap image collectÈe
